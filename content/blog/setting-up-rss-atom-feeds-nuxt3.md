...
image: /images/rss-atom-feed.jpg
alt: RSS and Atom feeds in Nuxt 3
postDate: 2025-08-31
tags:
  - Nuxt
  - web development
...
RSS (Really Simple Syndication) and Atom are XML formats for sharing new posts with readers. Instead of visiting each site by hand, readers subscribe to a feed. When a site publishes new content, the feed updates and readers see it in their feed reader.

Some popular web-based feed readers are FreshRSS, Feedly, Inoreader, and NewsBlur. Desktop apps like Fluent Reader and mobile apps like Reeder and Readrops are also available. I run a [FreshRSS](https://freshrss.org/) instance in my homelab and read updates through its web UI or the Readrops app on my phone.
...
After seeing how social media algorithms favor engagement over user interests, some of us started going back to RSS. While it's not perfect, I prefer the straightforward approach of choosing what I want to read rather than having it chosen for me.

## Adding feeds to my blog

Since I started using RSS feeds again, I wanted my Nuxt 3 blog to support both feed formats. I went with both formats because some feed readers are picky. Since the library generates the feeds, the extra few lines of code don't add much.
...
It took me a while to figure this out: on the server-side, we also need to pass the `event` parameter to `queryCollection` to access the server request context. Client-side code doesn't need this.
...
My RSS feed is available at `/rss.xml` and my Atom feed at `/atom.xml`.
...
I put RSS and Atom feed links in my website's footer with a familiar RSS icon. Visitors can find and subscribe to my feeds through their preferred reader.

## Static site generation

Since my blog is static, I needed to generate the feeds at build time. I added this to my `nuxt.config.ts`:
...
Adding RSS and Atom feeds to my Nuxt 3 blog took a library and a few short files. Now my readers can follow my updates in their preferred feed readers, just like I follow other blogs.

The full code is in my [GitHub repository](https://github.com/SubodhDahal/subodhdahal.github.io).
